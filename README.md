...
Este repositório ainda está em evolução. O foco implementado até agora é o **Wallet Service**, com domínio, casos de uso, persistência PostgreSQL/Prisma, endpoints REST e testes automatizados.
...
- Casos de uso:
  - criação de carteira;
  - consulta de carteira por `playerId`;
  - débito idempotente de aposta;
  - crédito idempotente de cashout.
- Persistência:
  - PostgreSQL;
  - Prisma;
  - atualização atômica de saldo + ledger de transações;
  - `operationId` único para impedir reprocessamento financeiro.
- API REST atual:
  - `GET /health`
  - `POST /wallets`
...
- Testes:
  - unitários de domínio e use cases;
  - integração do repositório Prisma com PostgreSQL;
  - e2e dos endpoints REST, incluindo idempotência e erros principais.

### Decisões técnicas relevantes

- Valores monetários são tratados em **centavos inteiros** com `bigint` no domínio e persistência.
- No HTTP, valores monetários são enviados e retornados como `string`, porque JSON não serializa `bigint`.
- `operationId` é a chave de idempotência das operações financeiras. Reenviar a mesma operação retorna a transação existente e não altera o saldo novamente.
- O Wallet Service é a fonte da verdade financeira. O Game Service deve solicitar débito/crédito, mas não decidir saldo.
- Os endpoints `debit-bet` e `credit-cashout` existem para validar o Wallet Service enquanto a integração Game -> Wallet via RabbitMQ ainda não foi implementada.

---

## Como Avaliar o Wallet Service

### Pré-requisitos

- Bun instalado.
- Docker e Docker Compose.
- PostgreSQL do projeto rodando localmente.
...
Em um banco novo, gere o client Prisma e aplique as migrations:

```bash
cd services/wallets
DATABASE_URL=postgresql://admin:admin@localhost:5432/wallets bunx prisma generate
DATABASE_URL=postgresql://admin:admin@localhost:5432/wallets bunx prisma migrate deploy
```

Volte para a raiz antes de rodar os scripts de validação:
...
Para rodar toda a validação do Wallet Service:
...
Os testes de integração e e2e usam `DATABASE_URL=postgresql://admin:admin@localhost:5432/wallets`, porque são executados na máquina host. O `.env` do serviço usa o host `postgres`, que é apropriado para execução dentro do Docker Compose.

### Rodar o Wallet Service localmente
...
Também há exemplos manuais em:

```text
services/wallets/requests/routes.controller.http
```
